...
This is a standalone break-glass tool, not an OpenShift console plugin --
unlike everything else under `plugins/`, it has no frontend and doesn't run
inside the console. It's built and shipped as its own container image so it
can be referenced directly from a pod spec.
...
via Docker/buildx -- see the root `Makefile`'s `build-node-terminal` /
`push-node-terminal` targets, or directly:

```bash
docker buildx build --platform=linux/amd64,linux/arm64 \
  --tag=<your-tag> --push plugins/node-terminal
```
