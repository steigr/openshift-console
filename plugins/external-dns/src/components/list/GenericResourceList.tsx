...
import { ExternalDNSModel } from '../../models';
...
type GenericResourceListProps<T extends K8sResourceCommon> = {
  model: ExternalDNSModel;
  namespace?: string;
  // Called with the full watched+filtered dataset once per render, so
  // columns that need a single batched lookup across all rows (e.g. DNS
  // record resolution) can compute it up front instead of per row.
...
function GenericResourceList<T extends K8sResourceCommon>({
  ...
  const { t } = useTranslation('plugin__external-dns-console-plugin');
