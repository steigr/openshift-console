...
const pluginJournalAPI = (): string =>
  consolePath('/api/proxy/plugin/logging-console-plugin/api/v1/nodes');

const PATCH_FLAG = 'LOGGING_JOURNAL_FETCH_PATCH';

type PatchedFetch = typeof window.fetch & {
  __loggingJournalPatch?: boolean;
};
...
type GuardedWindow = Window & {
  __loggingPathSelectGuard?: boolean;
  __loggingRawLinkRewriter?: boolean;
};
...
export const installRawLinkRewriter = () => {
  if ((window as GuardedWindow).__loggingRawLinkRewriter) {
    return;
  }
  (window as GuardedWindow).__loggingRawLinkRewriter = true;
  ...
  const rewriteAnchor = (anchor: HTMLAnchorElement) => {
    ...
    if (rewritten) {
      ...
      console.info(
        `[logging-console-plugin] rewrote raw journal link to "${rewritten}"`,
      );
    }
  };
  ...
  scan(document.body);
  console.info('[logging-console-plugin] raw journal link rewriter installed');
};
...
export const installPathSelectGuard = () => {
  if ((window as GuardedWindow).__loggingPathSelectGuard) {
    return;
  }
  (window as GuardedWindow).__loggingPathSelectGuard = true;
  ...
  document.addEventListener(
    ...
    (event) => {
      try {
        ...
        console.info(
          `[logging-console-plugin] suppressed re-select of active log path "${selectedPath}" (works around the never-resolving loading state in core NodeLogs)`,
        );
  ...
  console.info('[logging-console-plugin] path-select guard installed');
};
...
export const installJournalFetchPatch = (setFeatureFlag: SetFeatureFlag) => {
  ...
  if ((window.fetch as PatchedFetch).__loggingJournalPatch) {
  ...
  patched.__loggingJournalPatch = true;
