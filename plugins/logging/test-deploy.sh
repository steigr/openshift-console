...
set -euo pipefail
IMAGE=steigr/logging-plugin
TAG=ts-$(date +%s)
make push TAG=$TAG IMAGE=$IMAGE
helm upgrade --install console-logging-plugin charts/console-logging-plugin --debug --set=image.repository=$IMAGE --set=image.tag=$TAG --values=charts/console-logging-plugin/values-testing.yaml --wait
sleep 10
PLUGIN_INSTANCE_NAME=logging-console-plugin
